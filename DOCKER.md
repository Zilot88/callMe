# 🐳 Docker Deployment Guide

## Быстрый старт

### Шаг 1: Убедитесь, что SSL сертификаты созданы
```bash
# Проверьте наличие сертификатов
ls ssl/
# Должны быть: cert.pem, key.pem, openssl.cnf
```

Если сертификатов нет, создайте их:
```bash
mkdir -p ssl
cd ssl
openssl req -x509 -newkey rsa:2048 -nodes -sha256 -keyout key.pem -out cert.pem -days 365 -config openssl.cnf
cd ..
```

### Шаг 2: Соберите и запустите контейнер
```bash
# Вариант 1: Используя npm скрипты
npm run docker:build    # Собрать образ
npm run docker:up       # Запустить в фоне

# Вариант 2: Напрямую через docker-compose
docker-compose build
docker-compose up -d
```

### Шаг 3: Проверьте статус
```bash
# Посмотреть логи
npm run docker:logs
# или
docker-compose logs -f

# Проверить статус контейнера
docker ps
```

### Шаг 4: Откройте в браузере
```
https://localhost:8000
https://192.168.50.57:8000
https://176.36.188.208:8000
```

## 📋 Доступные команды

| Команда | Описание |
|---------|----------|
| `npm run docker:build` | Собрать Docker образ |
| `npm run docker:up` | Запустить контейнер в фоне |
| `npm run docker:down` | Остановить и удалить контейнер |
| `npm run docker:logs` | Показать логи (Ctrl+C для выхода) |
| `npm run docker:restart` | Перезапустить контейнер |
| `npm run docker:rebuild` | Пересобрать и перезапустить |

## 🔧 Настройка портов

По умолчанию приложение использует порт **8000** для HTTPS.

### Изменить порт

В файле `docker-compose.yml`:
```yaml
ports:
  - "8000:8000"  # Измените первое число на нужный порт
```

Например, для порта 8443:
```yaml
ports:
  - "8443:8000"
```

Затем пересоберите:
```bash
npm run docker:rebuild
```

## 🌐 Доступ из внешней сети

### Проброс через роутер

Убедитесь, что порт **8000** проброшен в настройках роутера:

1. Откройте настройки роутера (обычно 192.168.1.1 или 192.168.0.1)
2. Найдите "Port Forwarding" или "Проброс портов"
3. Добавьте правило:
   - **External Port**: 8000
   - **Internal Port**: 8000
   - **Protocol**: TCP
   - **Internal IP**: 192.168.50.57 (ваш локальный IP)

### DMZ (альтернатива)

Или используйте DMZ (если уже настроено):
- DMZ Host: 192.168.50.57
- Весь входящий трафик будет направлен на этот IP

## 🔒 SSL Сертификаты

### Расположение
```
./ssl/
  ├── cert.pem    # Публичный сертификат
  ├── key.pem     # Приватный ключ
  └── openssl.cnf # Конфигурация
```

### Обновление сертификатов

Сертификаты монтируются как volume, поэтому можно обновить без пересборки:

1. Создайте новые сертификаты в папке `ssl/`
2. Перезапустите контейнер:
   ```bash
   npm run docker:restart
   ```

### Использование Let's Encrypt

Для production рекомендуется использовать Let's Encrypt:

1. Получите домен (например, callme.example.com)
2. Используйте Certbot:
   ```bash
   certbot certonly --standalone -d callme.example.com
   ```
3. Скопируйте сертификаты:
   ```bash
   cp /etc/letsencrypt/live/callme.example.com/fullchain.pem ssl/cert.pem
   cp /etc/letsencrypt/live/callme.example.com/privkey.pem ssl/key.pem
   ```
4. Перезапустите:
   ```bash
   npm run docker:restart
   ```

## 🐳 Продвинутое использование

### Запуск без docker-compose

```bash
# Собрать образ
docker build -t callme:latest .

# Запустить контейнер
docker run -d \
  --name callme-app \
  -p 8000:8000 \
  -v $(pwd)/ssl:/app/ssl:ro \
  -e NODE_ENV=production \
  -e PORT=8000 \
  -e HOSTNAME=0.0.0.0 \
  --restart unless-stopped \
  callme:latest
```

### Использование переменных окружения

Создайте файл `.env`:
```env
PORT=8000
NODE_ENV=production
HOSTNAME=0.0.0.0
```

Обновите `docker-compose.yml`:
```yaml
services:
  callme:
    env_file:
      - .env
```

### Подключение к логам

```bash
# Показать последние 100 строк
docker logs callme-app --tail 100

# Следить за логами в реальном времени
docker logs callme-app -f

# С временными метками
docker logs callme-app -f -t
```

### Проверка состояния

```bash
# Статус контейнера
docker ps

# Использование ресурсов
docker stats callme-app

# Информация о контейнере
docker inspect callme-app
```

## 🔄 CI/CD Integration

### GitHub Actions пример

```yaml
name: Build and Deploy

on:
  push:
    branches: [ main ]

jobs:
  deploy:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v2

      - name: Build Docker image
        run: docker-compose build

      - name: Deploy
        run: docker-compose up -d
```

## 📊 Мониторинг

### Health Check

Docker Compose настроен с health check:
```bash
# Проверить здоровье контейнера
docker inspect --format='{{.State.Health.Status}}' callme-app
```

Статусы:
- `healthy` - Работает нормально
- `unhealthy` - Есть проблемы
- `starting` - Только запустился

### Метрики

```bash
# CPU, память, сеть
docker stats callme-app --no-stream
```

## 🛠 Устранение проблем

### Контейнер не запускается

```bash
# Проверьте логи
docker logs callme-app

# Проверьте ошибки сборки
docker-compose build --no-cache
```

### Порт уже занят

```bash
# Найдите процесс на порту 8000
# Windows:
netstat -ano | findstr :8000

# Linux/Mac:
lsof -i :8000

# Остановите старый контейнер
docker-compose down
```

### SSL сертификаты не найдены

```bash
# Убедитесь, что сертификаты существуют
ls -la ssl/

# Пересоздайте при необходимости
cd ssl
openssl req -x509 -newkey rsa:2048 -nodes -sha256 -keyout key.pem -out cert.pem -days 365 -config openssl.cnf
cd ..

# Перезапустите
npm run docker:restart
```

### Приложение недоступно из сети

1. Проверьте, что контейнер запущен:
   ```bash
   docker ps
   ```

2. Проверьте порты:
   ```bash
   docker port callme-app
   ```

3. Проверьте файрвол:
   ```bash
   # Windows
   netsh advfirewall firewall add rule name="CallMe" dir=in action=allow protocol=TCP localport=8000
   ```

4. Проверьте настройки роутера (проброс портов)

## 📦 Размер образа

Текущий образ использует многоэтапную сборку (multi-stage build):
- **Builder stage**: ~500MB (содержит все зависимости для сборки)
- **Runner stage**: ~200MB (только production зависимости)

Финальный образ будет ~200MB.

### Оптимизация размера

Образ уже оптимизирован:
- ✅ Использование Alpine Linux (минимальный размер)
- ✅ Multi-stage build
- ✅ Только production зависимости
- ✅ .dockerignore исключает ненужные файлы

## 🚀 Production рекомендации

1. **Используйте валидные SSL сертификаты** (Let's Encrypt)
2. **Настройте автоматический перезапуск**: `restart: unless-stopped`
3. **Мониторинг**: Настройте логирование и алерты
4. **Обновления**: Регулярно обновляйте образ
5. **Резервное копирование**: Сделайте backup ssl/ папки
6. **Безопасность**: Не публикуйте сертификаты в Git (добавьте в .gitignore)

## 📚 Дополнительные ресурсы

- [Docker Documentation](https://docs.docker.com/)
- [Docker Compose Documentation](https://docs.docker.com/compose/)
- [Next.js Docker Documentation](https://nextjs.org/docs/deployment#docker-image)
