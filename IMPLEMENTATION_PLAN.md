...
```bash
npm run dev  # Запускает server.js на порту 8000
```

Доступ:
- Локально: `http://localhost:8000`
- В локальной сети: `http://192.168.50.57:8000`
- Из интернета (через DMZ): `http://176.36.188.208:8000`

#### Доступ из локальной сети
Для доступа с других устройств в локальной сети:
1. Откройте `http://192.168.50.57:8000` на любом устройстве в сети
2. Все устройства автоматически присоединятся к общей конференции
3. Не нужно вводить коды или ID

#### Доступ из интернета
Настроен проброс порта 8000 через DMZ:
- Внешний IP: `176.36.188.208`
- Порт: `8000`
- Пользователи из интернета могут подключиться по адресу `http://176.36.188.208:8000`
